...
if __name__ == "__main__":
    ...
    for image_name in image_names:
        ...
        print(f"{image_name} - Showing original image. Press any key to continue...")
        cv2.waitKey(0)
        cv2.destroyAllWindows()

        # Step 2: Encrypt and show encrypted image
        encrypted_image, key = encrypt_image(input_image)
        cv2.imwrite(f"encrypted_{image_name}", encrypted_image)
        cv2.imshow("Encrypted", encrypted_image)
        print("Showing encrypted image. Press any key to continue...")
        cv2.waitKey(0)
        cv2.destroyAllWindows()

        # Step 3: Decrypt and show decrypted image
        decrypted_image = decrypt_image(encrypted_image, key)
        cv2.imwrite(f"decrypted_{image_name}", decrypted_image)
        cv2.imshow("Decrypted", decrypted_image)
        print("Showing decrypted image. Press any key to continue...")
        cv2.waitKey(0)
        cv2.destroyAllWindows()

        # Validation
        print(f"Processed {image_name}")
        if np.array_equal(input_image, decrypted_image):
            print(f"{image_name}: Decryption successful!")
        else:
            print(f"{image_name}: Decryption failed. Check intermediate outputs.")
